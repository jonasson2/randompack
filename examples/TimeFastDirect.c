...
static void init_fast_state(fast_state *st, uint64_t seed) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (st->s[i][0] == 0 && st->s[i][1] == 0 && st->s[i][2] == 0 && st->s[i][3] == 0)
      st->s[i][0] = 1;
  }
}
...
static inline __m256d u64_to_u01_4x_avx2(uint64_t r0, uint64_t r1, uint64_t r2, uint64_t r3) {
  __m256i v = _mm256_set_epi64x((long long)r3, (long long)r2, (long long)r1, (long long)r0);
  v = _mm256_srli_epi64(v, 12);
  v = _mm256_or_si256(v, _mm256_set1_epi64x((long long)0x3ff0000000000000ULL));
...
static double time_fast_u01(double out[], size_t len, double bench_time, fast_state *st) {
  ...
  return (calls > 0) ? 1e9 * (t - t0) / ((double)(calls * len)) : 0.0;
}
