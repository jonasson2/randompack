// -*- C -*-
// TimeFloat.c: time single precision distributions (ns/value).
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static void print_help(void) {
  printf("TimeFloat — time single precision distributions (ns/value)\n");
  printf("Usage: TimeFloat [options]\n\n");
  printf("Options:\n");
...
static bool fill_dist(float out[], int n, float param[], randompack_rng *rng) {
  size_t len = (size_t)n;
...
static void fill_wrapper(float out[], int n, float param[], randompack_rng *rng) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < LEN(dists); i++) {
    float par[3];
    par[0] = (float)dists[i].id;
    par[1] = dists[i].param[0];
    par[2] = dists[i].param[1];
    float x[4];
    fill_dist(x, 4, par, rng);
    double ns = time_float(chunk, bench_time, fill_wrapper, par, rng);
