...
for (i in 1:50000) runif(1000)
runif(chunk)
rnorm(chunk)
rexp(chunk, rate=1)
rweibull(chunk, shape=2, scale=1)
...
# gumbel(0,1)
ns = time_dist(function() -log(-log(runif(chunk))), chunk, reps, bench_time)
cat(sprintf("%-18s %8.2f\n", "gumbel(0,1)", ns))

# pareto(1,2)  (Type I, xm=1, alpha=2)
ns = time_dist(function() (1 - runif(chunk))^(-1/2), chunk, reps, bench_time)
cat(sprintf("%-18s %8.2f\n", "pareto(1,2)", ns))
