...
This package provides R bindings to the C library Randompack, a random number generation
toolkit that also includes interfaces for Julia, Fortran, and Python. Randompack exposes a
collection of modern RNG engines, including xoshiro256++/**, PCG64 DXSM, sfc64, Philox,
and ChaCha20, together with a range of probability distributions, both integer and
...
delivers high throughput for bulk generation, typically running 2–3× faster than base R
and about 1.7× faster than dqrng (a modern, high-performance RNG package) for uniform,
normal, and exponential draws.
...
contains setup and development instructions, including details specific to the R
interface.
...
s <- rng$sample(20, 5)             # 5-element sample from 1:20 (without replacement)
```
...
rng <- randompack_rng("philox", bitexact=TRUE) # exact agreement with specified engine
```
