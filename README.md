...
on correctness and reproducibility. The library provides a range of
distributions, including raw bit-streams, bounded and interval-based integers,
permutations and sampling without replacement, uniform and normal variates, and
several other commonly used continuous distributions. All distribution
algorithms are implemented directly from their original or standard published
descriptions, with care taken to avoid undocumented shortcuts or ad-hoc
modifications.
...
(programming language/computer/compiler/OS/architecture) agree. For U(0,1),
N(0,1), Exp(1), and integer distributions the agreement is bit-identical (x == y
holds). For the remaining distributions, samples agree to within about 2 ulp. If
the `bitexact` parameter is set to `true` the agreement is bit-identical for all
distributions.
...
`meson_options.txt`. Then build with meson/ninja and issue TestU01Driver TODO
